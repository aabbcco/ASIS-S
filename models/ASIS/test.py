...
parser.add_argument('--log_dir', default='log_shapenet_orig', help='Log dir [default: log]')
...
parser.add_argument('--input_list', type=str, default='data/test_hdf5_file_list_Area8.txt', help='Input data list file')
parser.add_argument('--model_path', type=str, default='log1/epoch_99.ckpt', help='Path of model')
...
def test():
    with tf.Graph().as_default():
        ...
        for shape_idx in range(len_pts_files):
            ...
            cur_data,cur_group,cur_sem=provider.load_h5_data_label_seg(room_path)
            ...
            cur_data = cur_data[:, 0:NUM_POINT, :]#xyz
            cur_sem = np.squeeze(cur_sem)           #sem label
            cur_group = np.squeeze(cur_group)       #inst label
            ...
            if output_verbose:
                ...
                for i in range(pts.shape[0]):
                    ...
                    pts[i, 0], pts[i,1], pts[i, 2],sem_softmax[i, sem[i]], sem[i], ins[i]))
