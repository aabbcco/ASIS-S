...
import provider
print(BASE_DIR)
DATAROOT = os.path.abspath(os.path.join(BASE_DIR,"../../"))
print("DATAROOT: ",DATAROOT)
parser = argparse.ArgumentParser()
parser.add_argument('--test_area', type=int, default=8, help='The areas except this one will be used to estimate the mean instance size')
...
def estimate(area):
    ...
    train_file_list = provider.getDataFiles(os.path.join(DATAROOT,file_path)) 
    ...
    for h5_filename in train_file_list:
        cur_data, cur_group, _, cur_sem = provider.loadDataFile_with_groupseglabel_stanfordindoor(os.path.join(DATAROOT,h5_filename))
